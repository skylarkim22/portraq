...
린트 에러나 패키지 경계·순환 의존성 위반이 있으면 수정한다.
...
먼저 `qa-tester` 에이전트를 호출해 `pnpm dev`로 앱을 띄운 상태에서 골든 패스·경계 케이스·목업(`docs/mockups/`) 대비 시각적 일치 여부를 자동으로 검증한다. 🔴 항목이 있으면 수정 후 `qa-tester`를 다시 호출한다.

🔴가 없으면 사용자에게 최종 육안 확인을 요청한다 — qa-tester의 통과가 이 단계를 대체하지 않는다:
...
PR URL을 사용자에게 알려준다. Vercel GitHub 연동이 되어 있다면 Preview 배포 링크가 PR에 자동으로 달린다 — 별도 배포 명령을 실행하지 않는다. Production 배포(main 머지 이후)가 필요한 시점이 오면 `deploy-to-vercel` 스킬을 사용하되, 실행 전 사용자에게 먼저 확인한다.
