...
- `id` UUID PK, `ticker` TEXT (`assets` FK), `record_date` DATE(배당기준일 — 이 날 주주여야 배당 대상), `pay_date` DATE NULL(실제 배당금 지급일), `dividend_reason` TEXT NULL(배당사유, 예: "현금배당"), `amount` NUMERIC(주당 배당금/분배금, 원), `source` TEXT (`DART`/`SEIBRO`/`DATA_GO_KR`/`KODEX`/`TIGER`), `created_at`, `updated_at`
...
- **ETF 분배금은 공식 공개 API가 없다** — data.go.kr에 관련 데이터셋이 없고, SEIBRO 오픈플랫폼(`openplatform.seibro.or.kr`)은 법인 회원 전용이라 개인 프로젝트로는 가입 불가. 대신 운용사가 자사 홈페이지에 공개하는 "상품별 분배 현황" 파일을 사람이 다운받아 `scripts/load-{운용사}-etf-dividends.mjs`로 적재한다(KR 배당 배치와 동일하게 보유 티커만 걸러 upsert, `--dry-run` 지원). 현재 KODEX(삼성자산운용, `scripts/load-kodex-etf-dividends.mjs`, 실제 `.xls` 바이너리, 종목코드 있음)와 TIGER(미래에셋자산운용, `scripts/load-tiger-etf-dividends.mjs`, 확장자만 `.xls`인 HTML 테이블·종목코드 없어 `assets.name`으로 역매칭, 최근 분배 발생 상품만 담긴 스냅샷이라 전체 이력은 아님)가 있고, ACE/SOL 등 다른 운용사는 파일을 받는 대로 같은 패턴으로 추가한다 — 운용사마다 파일 형식이 달라 공통 파서로 묶지 않는다
