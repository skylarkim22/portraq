...
- ETF/개별주식 분배금은 자동 배치 대신 **가끔 사람이 수동으로 SEIBRO/DART 원본을 받아와 갱신**하는 방식을 유지한다(평균 매수 단가 대비 예상 분배율 계산이 목적이라 매일 자동 갱신될 필요가 없다고 판단, #75). `scripts/input/seibro-etf-distributions-raw.json`(SEIBRO ETF 분배 원본, 3,453건)은 재수집 번거로움 때문에 git에 그대로 보존해 두고, 재갱신 시 이 파일을 새로 받아와 교체한 뒤 `scripts/load-etf-dividend-amounts.mjs`(금액 INSERT)와 `scripts/backfill-seibro-dividend-details.mjs`(pay_date=`TH1_PAY_TERM_BEGIN_DT`/dividend_reason=`RGT_RSN_DTAIL_NM` UPDATE)를 다시 돌려 생성된 SQL을 검토 후 마이그레이션으로 적용한다. DART 쪽은 `alotMatter`(주당현금배당금) API 자체에 지급일 개념이 없어 원래는 `pay_date`가 항상 NULL이었고, `dividend_reason`은 이 API 특성상 전부 `'현금배당'`으로 고정 백필했다(`20260823150000_backfill_dart_dividend_reason.sql`). `pay_date`는 이후 SEIBRO 배당내역상세 원본(`scripts/input/seibro-stock-dividends-raw.json`, `scripts/parse-seibro-stock-dividends-xls.mjs`로 xls→JSON 변환)을 (ticker, record_date) 기준으로 매칭해 일부 채웠다(`scripts/backfill-dart-dividend-pay-date.mjs`) — DART의 record_date(결산기준일)와 이 원본의 record_date(배정기준일) 개념이 달라 정확히 일치하는 행만 채워지고(2,562건 중 839건), 나머지는 여전히 NULL이다
