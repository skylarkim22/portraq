...
import { tradeLogQueries, type EnrichedTradeLog } from "@/features/trade-log/queries";
...
export const useCreateTradeLog = () => {
  ...
  const listQueryKey = tradeLogQueries.list().queryKey;
  ...
  return useMutation({
    ...
    onMutate: async (input) => {
      await queryClient.cancelQueries({ queryKey: tradeLogQueries.all() });
      const previous = queryClient.getQueryData<EnrichedTradeLog[]>(listQueryKey);
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: tradeLogQueries.all() });
    },
  });
};
